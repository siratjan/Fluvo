...
1. **Reduzierte Zustandskette im Piloten:** `received` → `delivered` | `handed_over` | `cancelled`, dazu `ended_unpaid` **[VORSCHLAG]** (Statuslabel offen bis Q3). Die Bedeutungen:
...
- **Rest-Unterpunkte von Q13 werden NICHT mitgeschlossen**, sondern als eigene Fragen weitergeführt (sonst wird Q13 fälschlich als erledigt verbucht):
  - Verhältnis `delivered` ↔ „Abgerechnet" für Lieferungen ohne erfasste Zahlung — ist `delivered` das Ende oder folgt „Abgerechnet"?
  - „Niemand drückt geliefert": eine Liefer-Bestellung, bei der niemand „geliefert" auslöst, bliebe für immer in `received` → sichtbarer Abschlussweg am Tagesende nötig (sonst Halden in `received`).
