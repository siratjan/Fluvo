...
| Benanntes Ende einer nie abgeholten Bestellung = **`ended_unpaid`** (eigenes Ende, nicht Storno; eigenes Ereignis `order.ended_unpaid`) | [ENTSCHIEDEN Sirat 2026-09-23] | Mitschrift Runde 53; K3, ADR 0015 |
| **Fiskalische** Behandlung von `ended_unpaid` (produzierte, nie kassierte Ware) | [OFFEN] Q3 | open-questions.md Q3 |
...
- **Verträge / Events:** `order.handed_over` (Übergabe als fertig markiert), `order.paid`, für das Ende ohne Bezahlung `order.ended_unpaid` (eigenes Ereignis mit Pflichtgrund, getrennt vom Storno; K3, ADR 0015) (Namen vorläufig, K6 noch offen); `payments` (Briefing §5.2)
...
> **Beantwortet (Runde 53, K3/ADR 0015):** Eine **Abhol-Bestellung, die nie abgeholt wird**, bleibt in `received`, bis ein Mensch sie am Tagesende (FA-16) bewusst beendet. Der benannte Endzustand ist **`ended_unpaid`** (Ohne Bezahlung beendet) — ein **eigenes** Ende, getrennt vom Storno-Pfad, mit eigenem Ereignis `order.ended_unpaid` und Pflichtgrund ([ENTSCHIEDEN Sirat 2026-09-23], Mitschrift Runde 53). **Keine automatische Statusänderung durch das System.** Offen bleibt nur die **fiskalische** Behandlung → Q3 (Steuerberaterin).
