...
## 2026-09-23 · Runde 53: K3-Durchgang — `delivered` als Ende, offene Bestellungen am Tagesende, `ended_unpaid` als eigenes Ende

- **`delivered` ist das Ende einer Lieferung (Sirat, entschieden):** Im Piloten folgt nach `delivered` **kein** „Abgerechnet". Die Geldabrechnung der Lieferungen läuft auf **Papier** (ADR 0008). `settled` wird **später mit der Fahrer-App** zwischen `delivered` und dem Ende eingeschoben (ADR 0015 Punkt 4). Schließt Q13-Unterpunkt 1. [ENTSCHIEDEN Sirat 2026-09-23] → K3 (Statuszeile, Legende, „später"-Absatz, Offene Fragen), FA-15 (Nachbedingung/Zustandsübergänge).
- **Offene Bestellungen am Tagesende / „niemand drückt geliefert" (Sirat, entschieden):** Der **Tagesabschluss** (FA-16) zeigt alle Bestellungen, die noch in `received` stehen. Inhaber oder Annahme schließen **jede einzeln bewusst** ab: geliefert (`delivered`), storniert (`cancelled`) oder ohne Bezahlung beendet (`ended_unpaid`) — jeweils mit den passenden Rechten und Pflichtgründen. **Keine automatische Statusänderung durch das System, es wird nichts erfunden.** Schließt Q13-Unterpunkt 2. [ENTSCHIEDEN Sirat 2026-09-23] → K3 (Sonderfall „Offene Bestellungen am Tagesende", Übergangstabelle, Testszenarien-Hinweise), FA-16 (Schritt 3/3a, Testszenarien, Rückverfolgung).
- **Noch offen (Sirat NICHT entschieden):** Ob offene Bestellungen (`received`) den Tagesabschluss **blockieren** oder nur **deutlich angezeigt** werden, hat Sirat nicht entschieden. → als `Frage an Sirat` in K3 und FA-16 markiert; `doc-updater` in `open-questions.md` aufnehmen.
- **`ended_unpaid` / „Ohne Bezahlung beendet" ist ein eigener Endzustand (Sirat, entschieden):** **nicht** Storno mit Grund. Pflichtgrund, **eigenes Ereignis `order.ended_unpaid`**. Begründung: Schwund und Storno sollen **getrennt auswertbar** sein. Das **[VORSCHLAG]-Kennzeichen** am Statuslabel **entfällt**. Die **fiskalische** Behandlung bleibt offen (Q3, Steuerberaterin). [ENTSCHIEDEN Sirat 2026-09-23] → K3 (Legende, Übergangstabelle, Offene Fragen), FA-15 (Ausnahme 2a, Regeln, Verträge, Offene Fragen), FA-16 (3a, Testszenarien, Verträge).
- **K3-Stand:** Q13 (ADR 0015) und die Unterpunkte 1–2 sowie `ended_unpaid` als eigenes Ende und „quittiert als Merkmal" (ADR 0015) sind entschieden; Durchgang mit Sirat erfolgt (Runden 52–53). Offen bleiben: FA-01 9b „Rückruf nötig" (außerhalb der Durchstich-Spur), Q3 fiskalisch, die neue Frage zum Tagesabschluss (blockieren oder anzeigen). **Kreuzverhör und Abnahme stehen aus.**

## 2026-09-23 · Runde 54: K5-Durchgang — Options-/Varianten-Modell und geplante Preise

- **Extras mit größenabhängigem Aufpreis (Sirat, entschieden):** Beim Piloten kostet **dasselbe Extra je nach Größe unterschiedlich viel** (z. B. extra Zwiebel auf der kleinen Pizza anders als auf der großen). Modellierung: **eine Tabelle für Größen/Varianten je Artikel** (`menu_item_variants`, mit eigenem Preis je Größe) und ein **Aufpreis je Extra je Variante** als **direkter Wert** (`menu_option_variant_prices`) — **nicht** als Faktor- oder Staffelregel. Der Server rechnet den Aufpreis; die Bestellung friert die gewählte Variante und den Aufpreis ein. Damit ist die Frage „Options-/Varianten-Modell" (Runde 51, Q26) **geschlossen**. `menu_options` trägt keinen `surcharge_cents` mehr. [ENTSCHIEDEN Sirat 2026-09-23] → K5 (ER-Diagramm, Datenwörterbuch: `menu_item_variants`, neue `menu_option_variant_prices`, `order_items.variant_name_snapshot`/`options_snapshot`), FA-12 (Nachtrag Runde 54, Regeln, Testszenarien T15/T16, Frage geschlossen), FA-01/K4. *Klein offen:* Handhabung variantenloser Artikel/Extras (z. B. Getränke) — vor der ersten Migration bestätigen.
- **Keine geplanten Preisänderungen im Piloten (Sirat, entschieden):** Eine Preisänderung gilt **ab dem Speichern**; Bestellungen frieren ihren Preis ohnehin ein. Ein **künftig geplanter** Preis („gültig ab" in der Zukunft) ist im Piloten **nicht** vorgesehen und als **späterer Ausbau** vermerkt — **präzisiert/ersetzt** für den Piloten den „gültig ab"-Datumsteil aus Runde 47. Das Feld `menu_items.price_valid_from` **entfällt** (keine Wirkung ohne Zukunftspreise). [ENTSCHIEDEN Sirat 2026-09-23] → K5 (Feld entfernt, Frage geschlossen), FA-12 (Schritt 3, Ausnahme 3a, Regeln, T2, Nicht-Teil, Frage geschlossen), K6 (kein `menu.price_scheduled`).
- **Aufgeräumt (K5):** Frage „C B1 / dritte Datenklasse (c)" ist durch **ADR 0016 entschieden** — in `er-durchstich.md` und `datenwoerterbuch.md` als entschieden markiert (nur die Fristen A1–A6 bleiben offen). Offen bleibt die **Steuerberaterin-Frage** zu brutto/`tax_cents` (DB 4.1 / S1) mit dem Hinweis: bis zur Antwort wird mit brutto + `tax_cents` gebaut; vor dem ersten echten Einsatz muss die Antwort vorliegen, danach sind die Positionen unveränderlich.
- **K5-Stand:** Durchgang mit Sirat erfolgt (Runde 54, K5-Ausschnitt). **Kreuzverhör und Abnahme stehen aus.** Offen bleiben: Anwalt A1–A6, Steuerberaterin S1–S3, variantenlose Artikel/Extras (klein).

## 2026-09-23 · Runde 55: Tagesabschluss blockiert (Q31) und variantenlose Artikel (K5)

- **Tagesabschluss blockiert bei offenen Bestellungen (Sirat, entschieden, Q31):** Der **Tagesabschluss** wird **blockiert**, solange noch Bestellungen in `received` stehen. Der Abschluss ist erst möglich, wenn **jede** offene Bestellung bewusst beendet wurde: geliefert (`delivered`), storniert (`cancelled`) oder ohne Bezahlung beendet (`ended_unpaid`) — jeweils mit den bestehenden Rechten und Pflichtgründen. **Das System ändert keinen Status automatisch.** Schließt die in Runde 53 offen gelassene Detailfrage (blockieren vs. nur anzeigen). [ENTSCHIEDEN Sirat 2026-09-23] → K3 (Sonderfall + Offene Fragen), FA-16 (Ablauf 3/3a, Regel, Testszenario FA-16-T25, Offene Frage), ADR 0015 (Nachtrag Runde 55), `open-questions.md` (Q31).
- **Variantenlose Artikel (Sirat, entschieden, K5):** Es gibt **beides** — Artikel mit Größen und Artikel mit nur einem Preis (z. B. Getränke, Salat), die trotzdem Extras mit Aufpreis haben können. Umsetzung **ohne Sonderfall:** **Jeder Artikel hat mindestens eine Variante**; ein Artikel **ohne Größe** bekommt **genau eine Standardvariante**. Der Extra-Aufpreis hängt damit **immer** an einer Variante (`menu_option_variant_prices`). **`menu_items.base_price_cents` entfällt** — bei durchgängiger Variante wäre ein Grundpreis am Artikel eine zweite Preis-Wahrheit (Regel 9 „eine Speisekarte"). **DB-Invariante:** mind. eine Variante je aktivem Artikel, **im Kern beim Speichern** durchgesetzt (Standardvariante wird bei fehlender Größe automatisch angelegt). Schließt die „klein offen"-Restfrage aus Runde 54. *Nicht Teil der Aufgabe:* Oberflächen-Anzeige der Standardvariante ohne Größennamen → K10. [ENTSCHIEDEN Sirat 2026-09-23] → K5 (ER: `menu_items` ohne `base_price_cents`, `menu_item_variants`-Invariante; Datenwörterbuch), FA-12 (Nachtrag Runde 55, Regel, Frage geschlossen).
