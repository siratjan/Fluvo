...
| K3 | Zustandsmodell der Bestellung mit Übergangstabelle | [modelle/zustand-bestellung.md](modelle/zustand-bestellung.md) | Entwurf (2026-09-23) — Gegenlesen `architect` eingearbeitet; **Q13 entschieden (ADR 0015)**; **Rest-Unterpunkte 1–2 und `ended_unpaid` als eigenes Ende entschieden (Runde 53)**; **Tagesabschluss blockiert bei offenen Bestellungen entschieden (Runde 55, Q31)**; **Durchgang mit Sirat erfolgt** (Runden 52–55); Kreuzverhör und Abnahme stehen aus (offen: Q3 fiskalisch, FA-01 9b „Rückruf nötig") | AP-005 | — (neu) |
| K4 | Gesprächsdesign der KI: Fluss, Function Calls, Eskalation, Verbote, Testanrufe | [gespraech/](gespraech/) | leer | AP-006 | — (neu) |
| K5 | Datenmodell (ER) + Datenwörterbuch mit DSGVO/GoBD-Klasse | [modelle/er-durchstich.md](modelle/er-durchstich.md), [vertraege/datenwoerterbuch.md](vertraege/datenwoerterbuch.md) | Entwurf (2026-09-23; 26 Tabellen, Durchstich-Ausschnitt) — Gegenlesen `database-reviewer` + `compliance-guard` eingearbeitet; **Datenklassen entschieden (ADR 0016)**; **Runde 54 eingearbeitet (Options-/Varianten-Modell + `menu_option_variant_prices`, keine geplanten Preise → `price_valid_from` entfällt); Runde 55 eingearbeitet (variantenlose Artikel → jeder Artikel mind. eine Variante/Standardvariante, `base_price_cents` entfällt); Durchgang mit Sirat erfolgt (Runden 54–55)**; Kreuzverhör und Abnahme stehen aus; offen: Anwalt A1–A6, Steuerberaterin S1–S3 | AP-007 | — (neu) |
